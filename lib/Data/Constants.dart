
class Constants {

  static  String  ALL_CHARCTERS_KEY= 'all';

  static  String  NAME_OF_CHARACTER_KEY= 'name';
  static  String  SPECIES_OF_CHARACTER_KEY= 'species';
  static  String  IMAGE_OF_CHARACTER_KEY= 'image';
  static  String  STATUS_OF_CHARACTER_KEY= 'status';
  static  String  GENDER_OF_CHARACTER_KEY= 'gender';
  static  String  HTTP_RESULTS_KEY= 'results';
  static  String  RICK_AND_MORTY_DATA_KEY= 'rickAndMortyDataKey';
  static  String  RICK_AND_MORTY_API= "https://rickandmortyapi.com/api/character";
  static  String  SHOW_ALL_KEY= 'all';
  static  String  STATUS_ALIVE= 'Alive';
  static  String  STATUS_DEAD= 'Dead';
  static  String  GENDER_FEMALE= 'Female';
  static  String  GENDER_MALE= 'Male';

}